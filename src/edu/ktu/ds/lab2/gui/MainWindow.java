...
import edu.ktu.ds.lab2.Jonušas.PeopleGenerator;
...
public class MainWindow extends BorderPane implements EventHandler<ActionEvent> {
    ...
    private static ParsableSortedSet<Person> peopleSet;
    private static ParsableSortedSet<Person> peopleSet2;
    private PeopleGenerator peopleGenerator = new PeopleGenerator();
    private PeopleGenerator peopleGenerator2 = new PeopleGenerator();  
    ...
    private void treeGeneration(String filePath) throws ValidationException {
        ...
        if (filePath == null) {
            peopleArray = peopleGenerator.generateShuffle(sizeOfGenSet, sizeOfInitialSubSet, shuffleCoef);
            paneParam1.getTfOfTable().get(2).setText(String.valueOf(sizeOfLeftSubSet));
        } else { // Skaitoma is failo
            peopleSet.load(filePath);
            peopleArray = new Person[peopleSet.size()];
            int i = 0;
            for (Object o : peopleSet.toArray()) {
                peopleArray[i++] = (Person) o;
            }
            // Skaitant iš failo išmaišoma standartiniu Collections.shuffle metodu.
            Collections.shuffle(Arrays.asList(peopleArray), new Random());
        }

        // Išmaišyto masyvo elementai surašomi i aibę
        peopleSet.clear();
        Arrays.stream(peopleArray).forEach(peopleSet::add);

        // Išvedami rezultatai
        // Nustatoma, kad eilutės pradžioje neskaičiuotų išvedamų eilučių skaičiaus
        KsGui.setFormatStartOfLine(true);
        KsGui.oun(taOutput, peopleSet.toVisualizedString(tfDelimiter.getText()),
    ...
    private void treeAdd() throws ValidationException {
        KsGui.setFormatStartOfLine(true);
        Person person = peopleGenerator.takeCar();
        peopleSet.add(person);
        paneParam1.getTfOfTable().get(2).setText(String.valueOf(--sizeOfLeftSubSet));
        KsGui.oun(taOutput, person, MESSAGES.getString("setAdd"));
        KsGui.oun(taOutput, peopleSet.toVisualizedString(tfDelimiter.getText()));
        KsGui.setFormatStartOfLine(false);
    }
    ...
    private void treeRemove() {
        ...
        if (peopleSet.isEmpty()) {
            ...
            int nr = new Random().nextInt(peopleSet.size());
            Person person = (Person) peopleSet.toArray()[nr];
            peopleSet.remove(person);
            KsGui.oun(taOutput, person, MESSAGES.getString("setRemoval"));
            KsGui.oun(taOutput, peopleSet.toVisualizedString(tfDelimiter.getText()));
        }
        KsGui.setFormatStartOfLine(false);
    }
    ...
    private void treeIteration() {
        ...
        if (peopleSet.isEmpty()) {
            ...
            KsGui.oun(taOutput, peopleSet, MESSAGES.getString("setIterator"));
        }
        KsGui.setFormatStartOfLine(false);
    }
    ...
    private void createTree() throws ValidationException {
        switch (cmbTreeType.getSelectionModel().getSelectedIndex()) {
            case 0:
                peopleSet = new ParsableBstSet<>(Person::new);
                break;
            case 1:
                peopleSet = new ParsableAvlSet<>(Person::new);
                break;
            default:
                disableButtons(true);
    ...
    private void treeLeaves(){
        KsGui.setFormatStartOfLine(true);
        int x = peopleSet.treeLeaves();
        KsGui.oun(taOutput, "Medžio lapų skaičius: " + x);
        KsGui.setFormatStartOfLine(false);
    }
    private void treeHeadSet(){
        KsGui.setFormatStartOfLine(true);
        KsGui.ounerr(taOutput, MESSAGES.getString("msg9"));
        Person person = (Person) peopleSet.toArray()[peopleSet.size()/2]; //vidurinis elementas
        BstSet<Person> newSet = (BstSet<Person>) peopleSet.headSet(person);
        
        KsGui.oun(taOutput, "Elementas");
        KsGui.oun(taOutput, person.toString());
        KsGui.oun(taOutput, "");
        KsGui.oun(taOutput, newSet.toString());
        KsGui.setFormatStartOfLine(false);
    }
